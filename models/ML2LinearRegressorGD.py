...
class LinearRegressorGD():
    ...
    def __init__(self, learning_rate=0.05, maxCycles=50000, minimumDelta = 0.005, stochastic=False):
    ...
    def makePredition(self, testX):
        return np.matmul(testX, self.weights)

    def gradient(self, index):
        return np.dot((self.makePredition(self.X[index]) - self.Y[index]), self.X[index])

    def updateWeight(self, error):
        delta = self.learning_rate * error
        self.weights = self.weights - delta
        self.cycles = self.cycles + 1
        return np.sum(np.absolute(delta))
    ...
    def train(self, data, features, label):
        ...
        while (self.cycles < self.maxCycles):
            ...
            for n in range(data.shape[0]):
                ...
                if (self.stochastic):
                    _delta = self.updateWeight(gradient)
                    if (_delta < self.minimumDelta):
                        return
                    gradient = 0

            if (not self.stochastic):
                _delta = self.updateWeight( (1/data.shape[0]) * gradient)
                if (_delta < self.minimumDelta):
                    return
    ...
    def test(self, testing_data, features, label):
        ...
        return self.makePredition(testX)
